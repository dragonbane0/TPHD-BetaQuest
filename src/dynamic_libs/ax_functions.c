...
void InitAcquireAX(void)
{
    ...
    if(OS_FIRMWARE >= 400) //sndcore2 was added with Firmware 4.0 and beyond
    {
        OSDynLoad_Acquire("sndcore2.rpl", &sound_handle);

		if (sound_handle) //Game uses sndcore2 since the library was available, use the functions from it to avoid crashes
		{
			AXInit = 0;
		...
		else //Game doesn't use sndcore2, fall back to snd_core
		{
			AXInitWithParams = 0;
