...
int cafiine_handshake(int sock) 
{
	...
	//Make sure iLock is 0 after a new connection to prevent deadlocks if the app is restarted while iLock is 1
	iLock = 0;
...
int getMode(int sock)
{
    ...
    CHECK_ERROR(sock == -1);
    {
		log_print("Check for Command");
		while (1)
		{
			ret = checkbyte(sock); //1 for dumping, 2 for injecting
...
error:
	log_print("Error while checking for Command");
    iLock = 0;
	ret = -1;
