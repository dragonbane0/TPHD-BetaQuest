...
static unsigned char exception_cb(void * c, unsigned char exception_type, int exceptionCode) 
{
	...
	*/
	...
	const unsigned char *CurrentStagePtr = (const unsigned char *)0x1064CDF6;
	const char *CurrentStage;

	const unsigned char *CurrentRoomPtr = (const unsigned char *)0x1064CE00;
	char *CurrentRoom;

	const unsigned char *CurrentSpawnPtr = (const unsigned char *)0x1064CDFF;
	char *CurrentSpawn;

	const unsigned char *CurrentStatePtr = (const unsigned char *)0x1064CE01;
	char *CurrentState;

	CurrentStage = ((const char *)CurrentStagePtr);
	CurrentRoom = ((char *)CurrentRoomPtr);
	CurrentSpawn = ((char *)CurrentSpawnPtr);
	CurrentState = ((char *)CurrentStatePtr);

	pos += sprintf(buf + pos, exception_print_formats[17], CurrentStage, *CurrentRoom, *CurrentSpawn, *CurrentState);
